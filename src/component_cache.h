...
public:
  int cache_full_number = 0;//新加入--统计cache满了几次
  ...
  bool manageNewComponent(StackLevel<T_num> &top, CacheableComponent<T_num> &packed_comp) {
   ...
   while(act_id){
     if (entry(act_id).equals(packed_comp)) {
       packed_comp.set_Hit(packed_comp.get_Hit() + 1);//新加入-记录该组件被hit
       packed_comp.set_flag(true);//新加入-被hit了更新flag
...
void rewrite_entry_score() //新加入-计算score并赋值
{
    for(unsigned id = 2 ; id < entry_base_.size(); id++)
    {
        if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() )
            entry_base_[id]->set_cache_score((int) entry_base_[id]->creation_time());
    }
}
void rewrite_entry_newin() //新加入-刷新新加入的参数
{
    for(unsigned id = 2 ; id < entry_base_.size(); id++)
    {
        if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() )
        {
            entry_base_[id]->set_cache_score(entry_base_[id]->get_cache_score()/2);
            if(entry_base_[id]->get_cache_score()==0)
            {
                entry_base_[id]->set_flag(false);
            }
            entry_base_[id]->set_rates(entry_base_[id]->get_Hit()/entry_base_[id]->get_num_variables_());
        }
  ...
  void component_refurbish_entry();//新加入-刷新entry条目
...
template<class T_num>
void ComponentCache<T_num>::component_refurbish_entry(){//新加入-刷新entry条目
  assert(statistics_.cache_full());
/*
    for (unsigned id = 2; id < entry_base_.size(); id++){
       if (entry_base_[id] != nullptr &&entry_base_[id]->isDeletable()&&entry_base_[id]->refurbish_entry(entry_base_[id])){
          //cout<<id<<" ";
...
*/
  ...
  for (auto it = entry_base_.begin() + 1; it != entry_base_.end(); it++)
    if (*it != nullptr && (*it)->isDeletable()) {
      scores.push_back((double) (*it)->get_cache_score());
    }
  sort(scores.begin(), scores.end());
  double cutoff = scores[scores.size() / 2];

  for (unsigned id = 2; id < entry_base_.size(); id++){//新加入综合评定-cache_score部分
    if (entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&(double) entry_base_[id]->get_cache_score() >= cutoff) {
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+100);
      }
    else if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&(double) entry_base_[id]->get_cache_score() < cutoff){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+50);
    }
    else if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&(double) entry_base_[id]->get_cache_score() == 0){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+0);
    }
  }
  
  for (unsigned id = 2; id < entry_base_.size(); id++){//新加入综合评定-flag部分
    if (entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&entry_base_[id]->get_flag() == true){
     entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+100);
    }
    else if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&entry_base_[id]->get_flag() == false){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+0);
    }
  }
    
  for (unsigned id = 2; id < entry_base_.size(); id++){//新加入综合评定-rates部分
    if (entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&entry_base_[id]->get_rates() > 0.3){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+100);
    }
    else if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&entry_base_[id]->get_rates() < 0.3 && entry_base_[id]->get_rates() > 0){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+50);
    }
    else if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&entry_base_[id]->get_rates() == 0){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+0);
    }
  }

  for (unsigned id = 2; id < entry_base_.size(); id++){//新加入综合评定-Hit部分
    if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&entry_base_[id]->get_Hit() > 0){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+100);
    }
    else if(entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&entry_base_[id]->get_Hit() == 0){
      entry_base_[id]->set_DYP_judge(entry_base_[id]->get_DYP_judge()+0);
    }
  }
  ...
  for (auto it = entry_base_.begin() + 1; it != entry_base_.end(); it++)
    if (*it != nullptr && (*it)->isDeletable()) {
      scores2.push_back((double) (*it)->get_DYP_judge());
    }
  sort(scores2.begin(), scores2.end());
  double cutoff2 = scores2[scores2.size() / 2];
  double cutoff3 = scores2[scores2.size()*0.8];
  if(cache_full_number<2){
     for (unsigned id = 2; id < entry_base_.size(); id++)//用新的评价变量去刷新一半的条目
         if (entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&(double) entry_base_[id]->get_DYP_judge() <= cutoff2) {
            removeFromDescendantsTree(id);
            eraseEntry(id);
         }
  }
  else{
    for (unsigned id = 2; id < entry_base_.size(); id++)//用新的评价变量去刷新80%的条目
         if (entry_base_[id] != nullptr &&entry_base_[id]->isDeletable() &&(double) entry_base_[id]->get_DYP_judge() <= cutoff3) {
            removeFromDescendantsTree(id);
            eraseEntry(id);
         }
  }
...
CacheEntryID ComponentCache<T_num>::storeAsEntry(CacheableComponent<T_num> &ccomp, CacheEntryID super_comp_id){
    ...
    if (statistics_.cache_full()){
        ...
        rewrite_entry_score();
        rewrite_entry_newin();
        component_refurbish_entry();
        //deleteEntries();
        cout<<"c o cache_full_number = "<<cache_full_number<<endl;
    }
    assert(!statistics_.cache_full());
