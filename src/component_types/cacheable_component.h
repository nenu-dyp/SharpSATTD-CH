...
public:
  ...
  bool change_flag(int cache_score){//新加入-更改flag
    if(cache_score==0){
      return false;
    }
  }
  
  float count_rates(int num_variables_,int Hit){//新加入-计算rates
      return Hit/num_variables_;
  }

  int get_cache_score(){//新加入-封装私有变量
    return cache_score;
  }
  
  void set_cache_score(int value){//新加入-封装私有变量
    cache_score=value;
  }

  bool get_flag(){//新加入-封装私有变量
    return flag;
  }
  
  void set_flag(bool value){//新加入-封装私有变量
    flag=value;
  }

  float get_rates(){//新加入-封装私有变量
    return rates;
  }
  
  void set_rates(float value){//新加入-封装私有变量
    rates=value;
  }

  int get_Hit(){//新加入-封装私有变量
    return Hit;
  }
  void set_Hit(int value){//新加入-封装私有变量
    Hit=value;
  }

  unsigned get_num_variables_(){//新加入-封装私有变量
    return num_variables_;
  }
  
  void set_num_variables_(unsigned value){//新加入-封装私有变量
    num_variables_=value;
  } 

  bool refurbish_entry(CacheableComponent<T_num> comp){//新加入-封装刷新函数
    if(comp.cache_score==0&&comp.flag==false&&comp.rates<=0.3){
      return false;
    }
  return true;
  }
  int get_DYP_judge(){//新加入-封装私有变量
    return DYP_judge;
  }

  void set_DYP_judge(int value){//新加入-封装私有变量
    DYP_judge=value;
  } 
private:  

  int DYP_judge = 0;//新加入的最终评价得分

  int cache_score = 0;//新加入-score

  bool flag = false;//新加入-flag

  float rates = 0;//新加入-rates

  int Hit = 0;//新加入-记录该组件被hit次数
